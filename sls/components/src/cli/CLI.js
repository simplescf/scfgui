...
class CLI {
  ...
  sessionStop(reason, messageOrError = 'Closed') {
    ...
    if (reason === 'error') {
      ...
      ipcRenderer.send('asynchronous-message',{type: 'senddeploylog', data: content, cate:'fin'})
  ...
  logError(error = {}, options = {}) {
    ...
    // Write to terminal
    ipcRenderer.send('asynchronous-message',{type: 'senddeploylog', data: content, cate:'log'})
  ...
  log(msg, color = null) {
    ...
    ipcRenderer.send('asynchronous-message',{type: 'senddeploylog', data: msg, cate:'sls'})
  ...
  logOutputs(outputs, indent = 0) {
    ...
    ipcRenderer.send('asynchronous-message',{type: 'senddeploylog', data: outputs, cate:'yml'})
    // Clear any existing content
    process.stdout.write(ansiEscapes.eraseDown);
